...
URDF_NAME = "../arcdog/urdf/arcdog.urdf"
# for climb hill
START_POS = [-1.5, -0.2, 0.45]
# for climb slope
# START_POS = [-1.5, -0.2, 0.8]

MPC_BODY_MASS = 14
MPC_BODY_INERTIA = np.array(
      (0.050874, -0.000451628, 0.000487603, 0.000451628, 0.64036, 4.8356e-05, -0.000487603, -4.8356e-05, 0.65655))
MPC_BODY_HEIGHT = 0.39
...
TOE_NAME_PATTERN = re.compile(r"\w+_foot_fixed\d*")
...
ALIENGO_DEFAULT_ABDUCTION_ANGLE = 0.1
ALIENGO_DEFAULT_HIP_ANGLE = 0.9
ALIENGO_DEFAULT_KNEE_ANGLE = -1.7
NUM_LEGS = 4
NUM_MOTORS = 12
...
class MotorModel(object):
  """A simple motor model for ALIENGO.
...
class SimpleRobot(object):
  def __init__(self, pybullet_client, robot_uid, simulation_time_step):
    ...
    self._motor_model = MotorModel(kp=self._kp, kd=self._kd, motor_control_mode=MOTOR_CONTROL_HYBRID)
    self._SettleDownForReset(reset_time=1.0)
    self._step_counter = 0
    import pybullet as p
    for j in range (p.getNumJoints(self.quadruped)):
        # hardcoded damping ratio here, if lower than this value might cause falling.
        p.changeDynamics(self.quadruped, j, linearDamping=0.1, angularDamping=0.1)
        p.changeDynamics(self.quadruped, j, lateralFriction=5000.)
